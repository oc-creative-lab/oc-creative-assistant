...
// 当前画布支持的 PoC 节点类型，需要与后端 DTO、节点工厂和 Vue Flow 插槽名保持一致。
export type CreativeNodeType = 'character' | 'plot' | 'worldbuilding' | 'idea' | 'research' | 'structure'

// 节点同步状态目前只是前端占位，用于表达未来 RAG/Agent 处理后的状态。
...
// 节点业务数据会随 graph 一起持久化；isActive 只是前端选中态，不写入后端。
